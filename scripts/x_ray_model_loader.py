...
class ModelLoader:
    def __init__(self, btch_size=32, img_size=(180, 180), color="grayscale", label_mode='categorical'):
        test_dir = pathlib.Path("data/test")
        pred_list = os.listdir("data/prediction/")

        test_ds = Dataset(test_dir, image_size=img_size, batch_size=btch_size, color_mode=color, label_mode=label_mode)
    ...
    def evaluate(self, binary=False):
        ...
        if(binary):
            test_loss, test_binary_accuracy, test_precision, test_recall = self.loaded_model.evaluate(self.x_test, self.y_test, verbose=2)
            ...
            test_loss, categorical_accuracy, test_precision, test_recall = self.loaded_model.evaluate(self.x_test, self.y_test, verbose=2)
            print('\nTest loaded loss is: %s' % (test_loss))
        ...
        if(binary):
            plot_roc_curve(self.y_test, y_pred, class_names=self.class_names, binary=True)
        else:
