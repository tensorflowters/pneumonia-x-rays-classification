...
class Model:
    ...
    def build(self):
        ...
        model.add(tf.keras.layers.RandomRotation(0.3, input_shape=(IMG_SIZE, IMG_SIZE, channels)))
        model.add(tf.keras.layers.RandomZoom(0.2))
        model.add(tf.keras.layers.RandomTranslation((-0.1, 0.1), (-0.1, 0.1)))
        model.add(
    ...
    def train(self, epochs, k=5):
        ...
        for train_index, val_index in kfold.split(self.x_train, self.y_train):
            ...
            model.save(MODEL_DIR.joinpath(f"model_6_fold_{fold_number}.keras"))
